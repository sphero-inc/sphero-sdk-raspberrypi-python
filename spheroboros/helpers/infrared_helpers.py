...
from spheroboros.helpers.infrared_helpers_enums import InfraredCodes
...
class InfraredControl:
    ...
    def __init__(self, rvr):
        if rvr is None:
            print('ERROR: PASS IN A RVR OBJECT')
            return
        self.__rvr = rvr
    ...
    async def start_infrared_broadcasting(self, far_codes, near_codes):
        ...
        try:
            ...
            print('ERROR: INPUTS MUST BE LISTS OF INFRARED CODES')
    ...
    async def start_infrared_following(self, far_codes, near_codes):
        ...
        :param far_code: List of InfraredCodes for far code
        :param near_code: List of InfraredCodes for near code
        :return:
        '''
    ...
    async def send_infrared_message(self, messages, strength=0):
        ...
        :return:
        '''
        if messages is None or strength < 0 or strength > 64 or len(messages) == 0:
            print('ERROR: NO MESSAGE SENT')
            return

        try:
            for message in messages:
                await self.__rvr.send_robot_to_robot_infrared_message(message.value,
                                                                      strength, strength, strength, strength)
        except TypeError:
            print("ERROR: INPUTS MUST BE A LIST OF INFRARED CODES AND AN INTEGER STRENGTH")
