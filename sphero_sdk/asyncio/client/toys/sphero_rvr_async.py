...
# Prefix:             RV
# Command Count:      77
# Timestamp:          03/31/2020 @ 21:00:11.873112 (UTC)
...
class SpheroRvrAsync(RvrFwCheckAsync): 
    ...
    async def drive_to_position_si(self, yaw_angle, x, y, linear_velocity, flags, timeout=None): 
    ...
    async def drive_to_position_normalized(self, yaw_angle, x, y, linear_velocity, flags, timeout=None): 
    ...
    async def stop_active_controller(self, deceleration_rate, timeout=None): 
        ...
        Args:
            ...
            timeout (float): maximum time to await a response.
        """
        command_dict = drive.stop_active_controller(deceleration_rate, target=2, timeout=timeout)
        return await self._dal.send_command(**command_dict)

    async def on_active_controller_stopped_notify(self, handler=None, timeout=None): 
        """Active controller has stopped async
