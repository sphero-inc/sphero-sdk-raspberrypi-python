...
# Prefix:             RV
# Command Count:      80
# Timestamp:          08/21/2019 @ 17:53:39.441034 (UTC)
...
class SpheroRvrAsync: 
    ...
    async def get_processor_name(self, target, timeout=None): 
        """Returns the processor name string (as specified to the System Info module). If no name is specified, returns
        an empty string or no string.
    ...
    async def on_collision_detected_notify(self, handler=None, timeout=None): 
        ...
        Args:
            handler (function): called asynchronously, takes form handler(accelerationX, accelerationY, accelerationZ, axis, powerX, powerY, speed, time).
            timeout (float): maximum time to await a response.
        Returns:
            Task (Future) from which `handler` will be called
        """
